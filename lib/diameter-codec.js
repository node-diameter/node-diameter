...
exports.constructRequest = function(applicationName, commandName, sessionId, options) {
    ...
    var request = {
        header: {
            ...
            flags: {
                request: true,
                proxiable: options && options.proxiable ? true : false,
                error: false,
...
var decodeAvpHeader = function(buffer, start) {
    var avp = {};
    if (buffer.length - start - DIAMETER_MESSAGE_AVP_CODE < 4) return null;
    avp.codeInt = buffer.readUInt32BE(start + DIAMETER_MESSAGE_AVP_CODE);
    if (buffer.length - start - DIAMETER_MESSAGE_AVP_FLAGS < 1) return null;
    ...
    if (buffer.length - start - DIAMETER_MESSAGE_AVP_LENGTH < 3) return null;
...
var decodeAvp = function(buffer, start, appId) {
    ...
    if (hasVendorId) {
        if (buffer.length - start - DIAMETER_MESSAGE_AVP_VENDOR_ID < 4) return null;
