...
var decodeAvpHeader = function(buffer, start) {
    ...
    if (buffer.length - start - DIAMETER_MESSAGE_AVP_LENGTH < 3) return undefined;
...
var decodeAvp = function(buffer, start, appId) {
    ...
    if (!avp) return undefined;

    var hasVendorId = avp.flags.vendor;
    if (hasVendorId) {
        if (buffer.length - start - DIAMETER_MESSAGE_AVP_VENDOR_ID < 4) return undefined;
