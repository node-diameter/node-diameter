...
exports.findApplication = function(applicationName) {
    var application;
    if (!_.isNumber(applicationName)) {
        application = diameterDictionary.getApplicationByName(applicationName);
    } else {
        application = diameterDictionary.getApplicationById(applicationName);
    }
    return application;
};

exports.findCommand = function(commandName, applicationId) {
    var command;
    if (!_.isNumber(commandName)) {
        command = diameterDictionary.getCommandByName(applicationId, commandName);
    } else {
        command = diameterDictionary.getCommandById(applicationId, commandName);
    }
    return command;
};
...
var encodeAvp = function(avp, appId) {
    var avpTag;
    if (!_.isNumber(avp[0])) {
        avpTag = diameterDictionary.getAvpByName(appId, avp[0]);
    } else {
        avpTag = diameterDictionary.getAvpByCode(appId, avp[0]);
    }
    if (avpTag == null) {
