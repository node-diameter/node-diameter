...
function DiameterCodec(diameterDictionary) {
    if (!(this instanceof DiameterCodec)) {
        return new DiameterCodec();
    }
    var self = this;
    self.dictionary = diameterDictionary;
    ...
    self.decodeMessageHeader = function(buffer) {
        var message = {
            _timeReceived: _.now(),
            header: {},
            body: []
    ...
    var inflateMessageHeader = function(message) {
        var command = self.dictionary.getCommandByCode(message.header.applicationId, message.header.commandCode);
        if (command == null) {
            throw new Error('Can\'t find command with code ' + message.header.commandCode);
        }
        message.command = command.name;
        var application = self.dictionary.getApplicationById(message.header.applicationId);
        if (application == null) {
    ...
    self.findApplication = function(applicationName) {
        var application;
        if (!_.isNumber(applicationName)) {
            application = self.dictionary.getApplicationByName(applicationName);
        } else {
            application = self.dictionary.getApplicationById(applicationName);
        }
        return application;
    };
    
    self.findCommand = function(commandName, applicationId) {
        var command;
        if (!_.isNumber(commandName)) {
            command = self.dictionary.getCommandByName(applicationId, commandName);
        } else {
            command = self.dictionary.getCommandById(applicationId, commandName);
        }
        return command;
    };
    
    self.constructRequest = function(applicationName, commandName, sessionId) {
        var application = self.findApplication(applicationName);
        var command = self.findCommand(commandName, application.id);
    ...
    self.constructResponse = function(message) {
        var response = {
            header: {
    ...
    var decodeAvp = function(buffer, start, appId) {
        ...
        var avpTag = self.dictionary.getAvpByCode(appId, avp.codeInt);
        if (avpTag == null) {
        ...
        if (avpTag.grouped) {
            ...
            var type = self.dictionary.resolveToBaseType(avpTag.type, appId);
    ...
    self.decodeMessage = function(buffer) {
        var message = self.decodeMessageHeader(buffer);
    ...
    var encodeAvp = function(avp, appId) {
        var avpTag;
        if (!_.isNumber(avp[0])) {
            avpTag = self.dictionary.getAvpByName(appId, avp[0]);
        } else {
            avpTag = self.dictionary.getAvpByCode(appId, avp[0]);
        }
        if (avpTag == null) {
        ...
        if (avpTag.grouped) {
            ...
            if (avpTag.enums) {
                var enumCode;
                if (!_.isNumber(value)) {
                    enumCode = _.find(avpTag.enums, {name: value});
                } else {
                    enumCode = _.find(avpTag.enums, {code: value.toString()});
                }
                if (enumCode == null) {
        ...
        if (avpTag['vendor-id']) {
            var vendorCode = self.dictionary.getVendorByVendorId(avpTag['vendor-id']).code;
    ...
    self.encodeMessage = function(message) {
...
exports.DiameterCodec = DiameterCodec;
