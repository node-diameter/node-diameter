...
function DiameterConnection(options, socket) {
        ...
        try {
            ...
            if (buffer.length >= DIAMETER_MESSAGE_HEADER_LENGTH_IN_BYTES) {
                ...
                while (buffer.length >= messageLength) {
                    const chunk = buffer.slice(0, messageLength);    // current message
                    buffer = buffer.slice(messageLength);    // rest of message
                    var message = diameterCodec.decodeMessage(chunk);
                    if (!message) continue; // drop
                    ...
                    if (buffer.length < DIAMETER_MESSAGE_HEADER_LENGTH_IN_BYTES) break;
                    messageLength = diameterCodec.decodeMessageHeader(buffer).header.length;    // length of next message
                }
            }
        } catch (err) {
    ...
    self.createRequest = function(application, command, sessionId, options) {
        if (sessionId === undefined) {
            sessionId = diameterUtil.random32BitNumber();
        }
        return diameterCodec.constructRequest(application, command, sessionId, options);
    };
