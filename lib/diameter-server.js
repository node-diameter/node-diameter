    ...
    this.tcpserver = net.createServer(function(socket) {
        ...
        socket.on('data', function(data) {
            ...
            if (bitsToBytes(buffer.length) >= DIAMETER_MESSAGE_HEADER_LENGTH_IN_BYTES) {
                var messageLength = diameterCodec.decodeMessageHeader(buffer).length;
                ...
                if (bitsToBytes(buffer.length) >= messageLength) {
                    diameterCodec.decodeMessage(buffer).then(function(message) {
                        self.emit(message.commandCode, message, diameterCodec.constructResponse(message), 
                            function(response) {
                            diameterCodec.encodeMessage(response).then(function(buffer) {
                                socket.write(buffer);
                            }, function(error) {
                                //handle
                                console.log(error);
